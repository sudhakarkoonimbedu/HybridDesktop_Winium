...
public class HybridDesktop_SharedResource {
	...
	@Before
	public void beforeTest(Scenario scenario) throws IOException, InterruptedException
	{			
		...
		}catch(Exception e)
		{
			...
			String[] str = {"Toolbar.exe","RxNova.exe","chromedriver.exe","Winium.Desktop.Driver.exe","Posclaim.exe","DMR.exe","Preauth.exe","Poserror.exe","Claim.exe","Member.exe","Group.exe","cust.exe","Client.exe","Prescrib.exe","PharmSim.exe","PharmPln.exe","PharmNet.exe","PharmFin.exe","Pha.exe","PharmPhm.exe","PharmGbl.exe","PharmCst.exe","PLANLIST.exe","Dedblpln.exe","Deduct.exe","Coverage.exe","PmntHist.exe"};
			for(String process:str)
			 {
				r.exec("Taskkill /IM "+process+" /F");
				Thread.sleep(500);
			 }			
...
//	}
	...
	{		
		...
		//getDriver().quit();
		//Tdriver.set(null);
		//String strVbsfileName = System.getProperty("user.dir") + "\\drivers\\EmailTrigger.vbs";
//		try {
//			Runtime.getRuntime().exec("wscript.exe " + System.getProperty("user.dir") + "/drivers/EmailTrigger.vbs");
//		} catch (IOException e) {			
//			e.printStackTrace();
//		}
	}	
